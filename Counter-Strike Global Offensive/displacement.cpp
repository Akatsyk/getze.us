...
namespace Engine::Displacement
{
	...
	bool Create()
	{
		...
		DT_BaseCombatCharacter::m_hActiveWeapon = pPropManager->GetOffset("DT_BaseCombatCharacter", "m_hActiveWeapon");

		auto m_hConstraintEntity = pPropManager->GetOffset("DT_BasePlayer", "m_hConstraintEntity");

		C_BasePlayer::m_pCurrentCommand = (m_hConstraintEntity - 0xC);//Memory::Scan( "client_panorama.dll", "89 ?? ?? ?? ?? ?? E8 ?? ?? ?? ?? 85 FF 75" ) + 2 );
		...
		DT_BasePlayer::m_lifeState = pPropManager->GetOffset("DT_BasePlayer", "m_lifeState");
		...
		//Data::m_pGlowManager = *reinterpret_cast<CGlowObjectManager**>(Memory::Scan("client_panorama.dll", "0F 11 05 ? ? ? ? 83 C8 01") + 3);
		//Data::m_uTraceLineIgnoreTwoEntities = *(DWORD**)(Memory::Scan("client_panorama.dll", "53 8B DC 83 EC ? 83 E4 ? 83 C4 04 55 8B 6B 04 89 6C 24 04 8B EC 81 EC 8C"));
		...
		//Data::m_uMoveHelper = **(std::uintptr_t**)(Memory::Scan("client_panorama.dll", "8B 0D ?? ?? ?? ?? 8B 45 ?? 51 8B D4 89 02 8B 01") + 2);
		//Data::m_uInput = *(std::uintptr_t*)(Memory::Scan("client_panorama.dll", "B9 ?? ?? ?? ?? F3 0F 11 04 24 FF 50 10") + 1);
		////Data::m_uGlobalVars = **( std::uintptr_t** )( Memory::Scan( "client_panorama.dll", "A3 ?? ?? ?? ?? E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? 6A" ) + 1 );
		//Data::m_uPredictionRandomSeed = *(std::uintptr_t*)(Memory::Scan("client_panorama.dll", "8B 0D ?? ?? ?? ?? BA ?? ?? ?? ?? E8 ?? ?? ?? ?? 83 C4 04") + 2); // 8B 0D ? ? ? ? BA ? ? ?? E8 ? ? ? ? 83 C4 04
		//Data::m_uUpdateClientSideAnimation = (DWORD)(Memory::Scan("client_panorama.dll", "55 8B EC 51 56 8B F1 80 BE ? ? 00 00 00 74 36"));
		//Data::m_uPredictionPlayer = *(std::uintptr_t*)(Memory::Scan("client_panorama.dll", "89 ?? ?? ?? ?? ?? F3 0F 10 48 20") + 2);
		Data::m_uClientBeams = *(IViewRenderBeams**)(Memory::Scan(cheat::main::clientdll, "B9 ? ? ? ? A1 ? ? ? ? FF 10 A1 ? ? ? ? B9") + 1);
		Data::m_uDirectX = **(std::uintptr_t**)(Memory::Scan("shaderapidx9.dll","A1 ? ? ? ? 50 8B 08 FF 51 0C") + 1);
