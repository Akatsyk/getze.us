...
void c_dormant_esp::draw_sound(C_BasePlayer* entity)
{
	...
	bool sound_expired = fabs(Source::m_pGlobalVars->realtime - sound_player.m_iReceiveTime) > 2.f;

	float meme = ((2.f - (Source::m_pGlobalVars->realtime - sound_player.m_iReceiveTime)) / 2.f) * 255.f;
	...
	if (cheat::Cvars.Visuals_fov_arrows.GetValue() && !cheat::main::local()->IsDead())
		cheat::features::visuals.draw_pov_arrows(entity, 256.f);
	...
	*(bool*)(uintptr_t(entity) + 0x93D) = true;
	...
	if (cheat::Cvars.Visuals_Box.GetValue()) {
		...
		Drawing::DrawOutlinedRect(x - 1, y - 1, w + 2, h + 2, Color(255, 255, 255, (meme * 0.8f)));
		Drawing::DrawOutlinedRect(x + 1, y + 1, w - 2, h - 2, Color(255, 255, 255, (meme * 0.8f)));
	}
	...
	if (cheat::Cvars.Visuals_Name.GetValue()) {
		...
		Drawing::DrawString(F::NewESP, x + w / 2 - text_size.right / 2, y - 13, Color(255, 255, 255, meme - 55.f), FONT_LEFT, "%s", info.name);
	}
	...
	if (cheat::Cvars.Visuals_Health.GetValue()) {
		...
		Drawing::DrawRect(x - 5, y - 1, 4, h + 2, Color(80, 80, 80, meme / 2));
		Drawing::DrawOutlinedRect(x - 5, y - 1, 4, h + 2, Color(10, 10, 10, (meme * 0.8f)));
		Drawing::DrawRect(x - 4, y + hp_percent, 2, h - hp_percent, Color(255, 255, 255, meme));

		if (hp < 90)
			Drawing::DrawString(F::ESPInfo, x - text_size.right - 6, y - 1, Color(255, 255, 255, meme - 55.f), FONT_LEFT, hps);
	...
	if (cheat::Cvars.Visuals_enemies_flags.Has(1) && resolver_info.fakeangles) {
		Drawing::DrawString(F::ESPInfo, x + w + 3, y + right++ * size_info, Color(255, 255, 255, meme - 55.f), FONT_LEFT, "FAKE");
		Drawing::DrawString(F::ESPInfo, x + w + 3, y + right++ * size_info, Color(255, 255, 255, meme - 55.f), FONT_LEFT, "R%d", resolver_info.resolving_method);
	}
}
