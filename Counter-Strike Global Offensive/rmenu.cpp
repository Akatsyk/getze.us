...
void CWindow::Draw()
{
	...
	// the lil cute eagle
	Drawing::DrawStringFont(Drawing::Eagle, m_x + 2, m_y + 2, Color::White(window_alpha), false, "=>");
	// the tittle
	if (m_DrawState)
	...
	if (m_DrawState == Default)
	{
		...
		// the lil x
		Drawing::DrawStringFont(Drawing::Eagle, m_x + m_w - 10, m_y + 2, Color::White(window_alpha), 1, "x");
	}
	...
	{
		// Welcome, user
		Drawing::DrawStringFont(Drawing::SegoeUI, m_x + m_w / 2, m_y + 30, Color::White(window_alpha), 1, "getgz.uz");
		// Line?
	...
	if (m_DrawState == Default && this->SelectedTab != nullptr)
	{
		...
		if (cheat::features::menu.Mousein(Vector(m_x + 5, m_y + 20,0), Vector(m_w / 3, 55,0)))
		{
			...
			//change arrow col
			Drawing::DrawStringFont(Drawing::hFont, m_x + 10, m_y + 60, arrowcol, false, "esc to exit");
			if (cheat::features::menu.mouse1_pressed && windows.at(windows.size() - 1) == this)
			{
				finalh = o_h;
				finalw = o_w;
		...
		Drawing::DrawStringFont(Drawing::BackArrow, m_x + 35, m_y + 42, arrowcol, 1, "x");
	...
	if (this->SelectedTab != nullptr)
	{
		...
		if (this->SelectedTab->Controls.size() > 0)
		{
			...
			if (this->SelectedTab->Controls.size() > 1)
			{
				...
				for (size_t i = 0; i < this->SelectedTab->Controls.size(); ++i)
				{
					auto font = this->SelectedTab->Controls[i].TabFont;

					// means use default tab font...

					if (font == 0 || this->SelectedTab->Controls[i].title.size() > 1)
						font = Drawing::SegoeUI;
					...
					Drawing::DrawStringFont(font, m_x + 3 + (i*TabSize + 1) + TabSize / 2.f, m_y + 95, Color::White(window_alpha), 1, this->SelectedTab->Controls[i].title.c_str());
				}
			}

			for (size_t i = 0; i < this->SelectedTab->Controls[tab].Controls.size(); ++i)
			{
...
void CCheckBox::Draw(bool hover)
{
	...
	// CheckMark
	Drawing::DrawStringFont(Drawing::Checkmark, pos.x + ((GetClickArea().x * 2 - 50) / 2), pos.y, Checkmark, false, "x");
	// Option name
...
{
	auto pos = GetPos();
	size_t Itemheight = 12;
	size_t ItemsToDraw = static_cast<size_t>(m_h) / Itemheight;
	size_t drawnItems = 0;
	...
	if (Items.size() > 0)
	{
		for (size_t i = ScrollTop; i < Items.size() && drawnItems < ItemsToDraw; i++)
		{
...
void CMultiComboBox::Draw(bool hover)
{
	...
	for (size_t lol = 0; lol < Selected.size(); lol++)
	{
...
void CVars::Save()
{
	...
	ini.SetDoubleValue("Eagle", "Spectatorlist.x", cheat::settings.radar_pos.x);
	ini.SetDoubleValue("Eagle", "Spectatorlist.y", cheat::settings.radar_pos.y);
