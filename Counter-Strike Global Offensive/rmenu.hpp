...
public:
	...
	std::string GetName() {
		if (!Items.empty() && SelectedIndex < (int)Items.size() && SelectedIndex >= 0)
...
public:
	...
	bool Has(int indx) { if (selected == 0) return false; indx = Math::clamp(indx, 0, (int)Selected.size()); return Selected[indx];/*std::find(Selected.begin(), Selected.end(), indx) != Selected.end();*/ }
...
public:
	...
	void RegisterTab(CTab* Tab, std::string title, std::string Icon = "Unknown", float h = 0, float w = 0, std::vector<std::string> tabs = { "" }, vgui::HFont TabFont = 0)
	{
		if (Tabs.size() == 0)
			SelectedTab = Tab;
		if (Tabs.size() > 0)
		...
		for (size_t i = 0; i < tabs.size(); ++i)
		{
			...
			Tab->Controls[i].TabFont = TabFont;
		}
...
public:
	...
	CGroupBox movement;
	// legit shit
	CGroupBox Legit_penis;
	CGroupBox Legit_Aimbot;
	CGroupBox Legit_useless;
	CCheckBox Legit_Enable;
	CCheckBox LegitBot_PositionAdjustment;
	CSlider   Legit_fov[6];
	CSlider   Legit_smooth[6];
	CComboBox Legit_hbotpriority[6];
	CComboBox Legit_hboxselection[6];
	CSlider	  Legit_rcsY[6];
	// filters
	CCheckBox Legit_head[6];
	CCheckBox Legit_chest[6];
	CCheckBox Legit_stomach[6];
	CCheckBox Legit_legs[6];
	CCheckBox Legit_arms[6];
	CKeyBind Legit_Key;
	CCheckBox Legit_ff;
...
#pragma endregion
	...
	CSlider		anti_aim_desync_range_right;
	...
	CComboBox	RageBot_SafePoints;
	CKeyBind	RageBot_BaimKey;

	CSlider		Visuals_spread_alpha;

	CMultiComboBox Visuals_indicators;

	CSlider		RageBot_PointScale_upperBody;

	CSlider		RageBot_PointScale_arms;
	CSlider		RageBot_PointScale_legs;

	CSlider		RageBot_DelayaShot;

	CComboBox	Visuals_hitsound;

	CCheckBox Visuals_unlock_invertory;

	CCheckBox Visuals_flashlight;
	CKeyBind	RageBot_flashlightkey;

	CCheckBox	RageBot_SafePointsBaimKey;

	CGroupBox	Legit_GMain;
	CCheckBox	LegitBot_Enabled;
	CCheckBox	LegitBot_OnKey;
	CKeyBind	LegitBot_Key;
	CCheckBox	LegitBot_IgnoreSmoke;
	CCheckBox	LegitBot_AutoPistol;
	CCheckBox	LegitBot_FriendlyFire;

	CGroupBox	Legit_Triggerbot;

	CCheckBox	LegitBot_Trigger_Enabled;
	CCheckBox	LegitBot_Trigger_OnKey;
	CKeyBind	LegitBot_Trigger_Key;
	CCheckBox	LegitBot_Trigger_IgnoreSmoke;
	CCheckBox	LegitBot_Trigger_AutoPistol;
	CCheckBox	LegitBot_Trigger_PositionAdjustment;
	CCheckBox	LegitBot_Trigger_FriendlyFire;

	CSlider		Legit_rcsX[6];

	CGroupBox Legit_Accuracy[5];

	CGroupBox Legit_Filter[5];
	CGroupBox Legit_Targetting[5];

	CSlider   Legit_firstshotdelay[6];
	CSlider   Legit_killdelay[6];

	CGroupBox Legit_TFilter[5];
	CCheckBox Legit_thead[6];
	CCheckBox Legit_tchest[6];
	CCheckBox Legit_tstomach[6];
	CCheckBox Legit_tlegs[6];
	CCheckBox Legit_tarms[6];

	CGroupBox Legit_TAccuracy[5];

	CSlider   Legit_TDelay[6];
	CSlider   Legit_THitchance[6];
	CSlider   Legit_TMinDamage[6];
	CCheckBox	LegitBot_Trigger_IgnoreWall[6];
	CCheckBox	LegitBot_Trigger_account_hitchance[6];

	CCheckBox Legit_SmoothRandomize[6];

	CMultiComboBox Visuals_misc_logthings;
	CCheckBox Ragebot_safe_pointselection;

	CSlider Visuals_misc_killtimer;

	CKeyBind RageBot_MinDmgKey;
	CSlider RageBot_MinDmgKey_val;

	CCheckBox Ragebot_fps_safety;

	CComboBox Misc_quickstop_type;
	CKeyBind	Misc_quickstop_key;

	CCheckBox	RageBot_scopeminwalk;

	CSlider		RageBot_accuracy_boost;
	CCheckBox	Misc_AutoFStop;
	CCheckBox Visuals_rem_sleeves;
	CCheckBox Visuals_rem_teammates;

	CCheckBox	anti_aim_desync_switch_limit;
	CCheckBox	RageBot_AutoStopBetweenShots;

	CCheckBox Visuals_rem_second_scope;

	CComboBox Misc_skins_glove_skin;
	CComboBox Misc_skins_glove_type;
	CComboBox Misc_skins_stattrak_type;

	CMultiComboBox RageBot_AntiAim_FakeAutoDirection;
	CCheckBox	anti_aim_desync_extend_limit_on_shot;

	CCheckBox Visuals_chams_resolve;
